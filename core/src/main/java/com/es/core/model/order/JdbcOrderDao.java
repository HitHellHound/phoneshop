...
import com.es.core.order.OrderRowMapper;
...
public class JdbcOrderDao implements OrderDao {
    ...
    public Optional<Order> getById(final Long key) {
        return namedParameterJdbcTemplate.getJdbcOperations()
                .query(GET_ORDER_BY_ID, orderRowMapper, key).stream().findAny();
    }

    @Override
    public Optional<Order> getBySecureID(String secureID) {
        return namedParameterJdbcTemplate.getJdbcOperations()
                .query(GET_ORDER_BY_SECURE_ID, orderRowMapper, secureID).stream().findAny();
    }
